...
function makeMessageChannel(chat) {
  return `${CHANNELS.ENCRYPTED_BASE}${chat.id}`;
}

export function connect(chats, userId, onMessage, onChat) {
  chats?.forEach(c => {
    const messageChannel = pusher.subscribe(makeMessageChannel(c));
  ...
  return () => {
    ...
    chats?.forEach(c => pusher.unsubscribe(makeMessageChannel(c)));
  };
}
